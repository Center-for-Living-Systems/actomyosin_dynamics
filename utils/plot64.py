# this code is to plot out example results
# extended based on original code with dimensional changes [28,28] => [64,64]
# Liya Ding
# 2024.03
...
def plot_AE(model, test_dataset):
    ...
    fig, axarr = plt.subplots(2, n, figsize=(n, 2))
    ...
    for i in range(n):
        axarr[0, i].axis('off')
        axarr[1, i].axis('off')
        axarr[0, i].imshow(x_input_sample[i],cmap=plt.cm.gray)
        axarr[1, i].imshow(x_output[i],cmap=plt.cm.gray)
    ...
    ax_dist.scatter(z1, z2, color='blue')
...
def plot_VAE(model, test_dataset):
    ...
    x_input_sample = x_input_sample.numpy().reshape([n, 64, 64])
    x_output = model.decode(z, apply_sigmoid=True).numpy().reshape([n, 64, 64])
...
def plot_CVAE(model, test_dataset):
    ...
    fig, axarr = plt.subplots(2, 5, figsize=(5, 2))
    x_input_sample = x_input_sample.numpy().reshape([n, 64, 64])
    x_output = model.decode(z, y_input_sample, apply_sigmoid=True).numpy().reshape([n, 64, 64])
    ...
    for i in range(num_classes):
        for j, z_j in enumerate(np.linspace(-3, 3, n)):
            ...
            generated_img = model.decode(z, [i], apply_sigmoid=True).numpy().reshape([64, 64])
            axarr[i, j].axis('off')
            axarr[i, j].imshow(generated_img, cmap='binary')
