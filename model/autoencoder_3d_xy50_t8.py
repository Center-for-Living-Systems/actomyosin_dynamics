# The 50*50*8 version of AE structures
...
def create_model():
  return tf.keras.models.Sequential([
    tf.keras.layers.Flatten(input_shape=(8, 50, 50, 1), name='layers_flatten'),
    ...
    def __init__(self, latent_dim: int, net_type:str='simple'):
        ...
        if net_type == "simple":
            self.inference_net = tf.keras.Sequential([
                InputLayer(input_shape=[8, 50, 50]),
                Flatten(),
            ...
            self.generative_net = tf.keras.Sequential([
                ...
                Dense(1024, activation='relu'),
                Dense(4096, activation='relu'),
                Dense(50 *50 * 8 * 1, activation='sigmoid'),
                Reshape(target_shape=[8, 50, 50]),
            ])
        if net_type == "conv":
            self.inference_net = tf.keras.Sequential([
